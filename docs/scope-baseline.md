...
3. `1.1.5` 개발 시작 승인 방식은 `Gate A blocker closed + Product Owner explicit approval`로 정의한다.
...
1. `Gate A blocker closed`
2. `Product Owner explicit approval`

둘 중 하나라도 빠지면 개발을 시작할 수 없다.

### 7.2 Meaning of Gate A Blocker Closed

`Gate A blocker closed`의 의미는 아래와 같다.

- Gate A checklist에서 구현 착수를 직접 막는 항목이 모두 닫혀 있다.
...
Product Owner가 개발 시작을 승인하기 전에 최소 아래 문서 패키지를 기준으로 검토한다.
...
실제 개발 시작 승인은 아래 방식으로 남긴다.

- Gate A review note 작성
- `docs/decision-log.md`에 승인 결정 기록
- `docs/WBS.md`와 `docs/roadmap.md` 상태 갱신

이 문서 자체는 개발 시작 승인이 아니다.  
이 문서는 승인 방식만 정의한다.
