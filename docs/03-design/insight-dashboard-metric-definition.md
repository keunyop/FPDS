...
- the public `/` Home route now suppresses low-value `recently_changed_30d` ranking presentation and the visible Recently Changed KPI card
- the public `/` Home route no longer renders the Products by type breakdown or an inline Data notes card; methodology and data-boundary details remain reachable through `/methodology`
- the public `/` Home route keeps decision-useful ranking cards, bank coverage, KPI summary, and optional like-for-like scatter when the selected product type supports it

Implemented public Home future-product wording and footer polish on `2026-06-09`:
- the public `/` Home route now uses product-neutral Home copy so the surface does not read as deposit-only while the API and current public catalog can still serve the active deposit scope
...
- the public `/` Home route now includes purpose-first entry cards that link into focused Product Grid states for everyday banking cost, savings-rate, fixed-term return, and low-entry-amount comparison paths
...
- the public `/` Home route no longer renders the snapshot eyebrow label, the purpose-entry explanatory subtitle, or the compact trust cue block for scope, snapshot freshness, and official-bank confirmation
