...
| `FPDS_SOURCE_BROWSER_FALLBACK_DOMAINS` | optional allowlist for domains that may use a local headless-browser PDF snapshot when normal HTTPS fetches are blocked/timed out, an explicit rate page returns only a JavaScript shell, or a product page exposes an unresolved dynamic rate placeholder. Current default baseline: `bmo.com,www.bmo.com,cibc.com,www.cibc.com,simplii.com,www.simplii.com,tangerine.ca,www.tangerine.ca` |
