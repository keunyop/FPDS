...
| `FPDS_SOURCE_BROWSER_FALLBACK_DOMAINS` | optional allowlist for domains that may use local headless-browser DOM for HTML-only discovery and PDF for snapshot capture when normal HTTPS fetches are blocked/timed out, an explicit rate page returns only a JavaScript shell, or a product page exposes an unresolved dynamic rate placeholder. The selected bank's exact fetch allowlist still applies. Current Canada default baseline: `bmo.com,www.bmo.com,cibc.com,www.cibc.com,rbcroyalbank.com,www.rbcroyalbank.com,simplii.com,www.simplii.com,tangerine.ca,www.tangerine.ca,vancity.com,www.vancity.com` |
| `FPDS_SOURCE_BROWSER_DOM_SNAPSHOT_DOMAINS` | optional subset whose browser fallback snapshot is retained as rendered HTML instead of PDF so structured CMS product/condition relationships survive parsing. Default: `vancity.com,www.vancity.com` |
