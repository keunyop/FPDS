...
- an allowlisted HTML-only discovery fetch may replace a blocked/timed-out direct response with browser-rendered DOM; snapshot capture uses browser-rendered PDF for the same transport failure or when an explicit rate page is only a JavaScript shell or a product page retains a recognizable unresolved rate placeholder. The requested output format is explicit, both remain on the same validated official URL and per-bank domain boundary, snapshot headers record the fetch method, and downstream missing-field validation still protects publication.
