...
- link exclusions are matched against URL/action-flow context and concise CTA labels, not arbitrary substrings in long product-card copy. A legitimate sentence containing `offers` remains eligible, while login, apply, compare, open-account, and explicit promotion paths remain excluded.
...
- a specifically named singular product page can survive plural navigation or cross-sell wording only when its title/primary heading and page evidence independently confirm that product identity. Generic marketing/action headings remain non-product identities.
- an official family overview may remain candidate-producing when bounded discovery cannot split its variants, but a generic identity plus multiple variant headings is persisted as `multi_product_family_overview`. Validation then emits error-level `ambiguous_product_boundary`, Review recommends `defer`, and auto-promotion independently refuses canonical publication. This applies to deposit and lending families without inventing product-specific values.
- plural generic identities such as `credit cards`, `loans`, `mortgages`, `savings accounts`, or `GICs` count as family identities when the page also exposes multiple named variants. Retail lending family pages remain evidence-only because their variant-specific fees, rates, limits, and eligibility cannot be safely collapsed into one candidate; deposit overviews may retain the existing review-first boundary only when they are explicitly identified as ambiguous and cannot auto-publish.
- explicit `/commercial/` paths or concise commercial product identities are hard retail-scope vetoes across product types, even when the page contains otherwise strong rate or term attributes.
...
- deposit supporting-link discovery excludes mutual-fund, fund-facts, simplified-prospectus, committed-capital, and governance/reporting documents. Those pages may contain valid investment percentages, but they are outside the selected deposit product boundary and cannot become rate evidence.
- mortgage refinance or renewal URLs are not excluded by name alone; they are rejected as `non_product_service_flow` only when the page identity also contains advice, explanation, or account-manager servicing signals. A specifically named refinance product can still pass normal detail evidence gates.
- the first `h1` is the primary page identity. Later `h1`, `h2`, and `h3` values remain separate section signals so navigation or repeated headings cannot be concatenated into a false product name.
- a singular named-product `h1` that agrees with the page title, has explicit product attributes, and is confirmed as detail by the bounded AI scorer may survive plural SEO-title or related-product headings at the ordinary detail threshold. A plural/category `h1` still remains a family overview.
- explicit application, prequalification, internal `shadow-site`, login, and account-opening flows are not public product evidence and are excluded before source materialization.
...
- promoted pages with the same bank, product type, confirmed page title, and confirmed primary heading collapse into one canonical detail source; locale or host variants are retained as aliases. Snapshot reuse is limited to the same `source_document` and checksum: byte-identical WAF or error bodies from different URLs must retain separate failure lineage instead of being treated as one successful product source.
