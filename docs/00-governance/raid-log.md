...
| R-004 | Medium | Open | Public aggregate refresh, snapshot freshness, and canonical truth can drift when operator collection or refresh work is delayed or queue/retry behavior regresses. | Keep dashboard health visibility, explicit collection/retry procedures, and latest-successful serving fallback in place. Use the D-078 overdue report for daily triage and weekly country/type reconciliation; manually re-verify product facts through existing approval before an aggregate refresh. Rebuilding alone cannot reset verification age. | Backend |
...
| R-007 | High | Monitoring | The live SwitchaBank Public site reaches Vercel API Preview/Production environments that temporarily share the development database, coupling anonymous reads, bounded product-counter writes, authenticated API operations, development work, capacity, and incident impact. | Treat this as a Product Owner-approved temporary exception only: use separate Preview/Production auth and Public-app secrets, route browser traffic through the Public BFF, validate product IDs against the active snapshot, rate-limit the counter writer, retain only daily aggregates for 400 days, monitor database/write behavior, and move Production to a separately provisioned database before the coupling becomes release-critical. | Product Owner, Backend |
| R-008 | High | Open | Two tracked current documents explicitly contained a deployment password; historical Git copies can retain it after current-text redaction. | Current literals removed. Security owner must verify actual use, rotate affected password/session-signing material, and inspect full Git history and the handover archive before transfer. No credential or deployment was changed in this review. | Product Owner; client Security owner to be named |
...
| A-005 | Medium | Monitoring | The initial Vercel API may serve the latest completed Public snapshot under operator-managed freshness for a bounded period. | Product Owner confirmation of acceptable snapshot age before Public launch and an explicit refresh plan when freshness becomes necessary. | Product Owner, Backend |
...
| I-004 | High | Open | Operator disable, all-session revocation, and password recovery lack a delivered operator lifecycle tool/runbook rehearsal. | Handover account-revocation and break-glass UAT cannot be signed off. | Approve the preview-first privileged CLI proposal in descent/02-release-readiness.md or provide and rehearse an approved client IAM/DBA process; implementation auto-review requested explicit authorization. | Product Owner; client System/Security owners to be named |
| I-005 | High | Open | Existing US Public product `prod_QSCmmE97KOSfGp_c` returns `rate.kind=absolute` and `comparable_rate=0.375`, while its public source text describes a Premier interest-rate reduction. Reproduced in the prior deposit QA snapshot and the read-only API during Loan Home QA on 2026-09-24. | Existing numeric consumers can rank a discount as a full mortgage rate; the Loan selector preserves the current API contract and does not correct this upstream discrepancy. | Separately verify official source semantics, add a rate-classification regression and use the authorized review/correction workflow for any persisted-data change. No canonical mutation or guessed replacement rate in the Home UI slice. | Product Owner, AI/Data, Backend |
...
| 2026-08-22 | Updated R-004 and added A-005 for the request-scoped Vercel Public-read period |
...
| 2026-08-28 | Aligned R-004, R-007, A-005, and I-002 with the Admin manual-only collection boundary |
| 2026-08-30 | Updated R-007 for bounded anonymous Public product-counter writes and their rate/retention controls |
