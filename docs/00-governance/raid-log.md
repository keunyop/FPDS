...
| I-005 | High | Monitoring | Official-source review on 2026-09-24 confirmed BofA product `prod_QSCmmE97KOSfGp_c` carried a discount as a full rate. The authorized bounded correction removed the scalar and deactivated its incomplete Public projection, preserving versions and source evidence. | The known bad record no longer ranks; the generic discount-language classification gap can recur in newly ingested records. | Add a focused classifier regression and correction in a separate runtime slice; see [bounded data report](home-top5-data-completion-2026-09-24.md). Do not infer a replacement full mortgage rate. | Product Owner, AI/Data, Backend |
