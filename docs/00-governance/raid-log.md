...
| R-004 | Medium | Open | Public aggregate refresh, snapshot freshness, and canonical truth can drift when operator collection or refresh work is delayed or queue/retry behavior regresses. | Keep dashboard health visibility, explicit collection/retry procedures, and latest-successful serving fallback in place. Perform an approved manual refresh before freshness becomes release-relevant. | Backend |
