...
- Public deposit Home/finder comparisons now match currency, explicit annual/APY
  basis and exact GIC term/redemption conditions. The calculator uses explicit
  scenarios or concise unavailable reasons; reviewed Oaken starts at 10,000.
  INDEXED GIC linked returns and inconsistent terms are excluded from arithmetic.
  See `docs/03-design/public-deposit-comparison-policy.md` for current data limits.
...
- `WBS 5.25` delivered guarded collection completion and a first-class Public credit-card catalog; current Admin collection and recovery are operator-initiated
...
- working prototype ingestion code for discovery, preflight drift checks, operator-initiated registry refresh artifacts, snapshot capture, parse/chunk, and evidence retrieval stages
...
- discovery preflight drift checks and operator-initiated refresh artifact generation are now available under `worker/discovery/`
