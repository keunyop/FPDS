...
from .models import ValidationArtifactLookup, ValidationResult, ValidationRoutingConfig

_SCHEMA_IDENTIFIER_RE = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*$")


@dataclass(frozen=True)
class ValidationRoutingDatabaseConfig:
    database_url: str
    schema: str

    @classmethod
    def from_env(cls) -> "ValidationRoutingDatabaseConfig":
        database_url = os.getenv("FPDS_DATABASE_URL")
        if not database_url:
            raise ValueError("FPDS_DATABASE_URL is required for validation/routing DB access")
...
@dataclass(frozen=True)
class ValidationRoutingPersistenceResult:
    run_id: str
    run_state: str
    candidate_count: int
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "auto_validated_count": self.auto_validated_count,
...
class PsqlValidationRoutingRepository:
    def __init__(
        self,
        config: ValidationRoutingDatabaseConfig,
        *,
...
"""
    ...
    def load_latest_normalization_artifacts(
        self,
        *,
...
FROM (
    SELECT DISTINCT ON (me.source_document_id)
        ...
        me.model_execution_id AS normalization_model_execution_id,
        me.execution_metadata ->> 'normalized_storage_key' AS normalized_storage_key,
        me.execution_metadata ->> 'normalized_metadata_storage_key' AS normalization_metadata_storage_key,
        sd.bank_code,
        sd.country_code,
        sd.source_type,
        sd.source_language,
...
"""
    ...
    def load_taxonomy_registry(self) -> dict[str, set[str]]:
        schema = self.active_schema
        sql = f"""
SET search_path TO {schema};

SELECT COALESCE(json_object_agg(product_type, subtype_codes), '{{}}'::json)::text
FROM (
    SELECT
        product_type,
        json_agg(subtype_code ORDER BY display_order, subtype_code) AS subtype_codes
...
"""
        output = self._execute(sql, variables={})
        payload = json.loads(output or "{}")
        return {str(key): {str(item) for item in value} for key, value in payload.items()}

    def load_routing_config(self, *, routing_mode: str) -> ValidationRoutingConfig:
        schema = self.active_schema
        sql = f"""
SET search_path TO {schema};

SELECT COALESCE(json_agg(row_to_json(policy_rows)), '[]'::json)::text
FROM (
...
"""
        ...
        policy_map = {str(row["policy_key"]): row["policy_value"] for row in rows}
        return ValidationRoutingConfig(
    ...
    def persist_validation_result(
        ...
        *,
        ...
        triggered_by: str | None,
        completed_at: str,
    ) -> ValidationRoutingPersistenceResult:
        schema = self.active_schema
        candidate_updates = [
            item.candidate_update_record
            for item in validation_result.source_results
            if item.candidate_update_record is not None
        ]
        review_tasks = [
            item.review_task_record
            for item in validation_result.source_results
            if item.review_task_record is not None
        ]
        ...
        auto_validated_count = sum(1 for item in validation_result.source_results if item.validation_action == "auto_validated")
        run_state = "failed" if source_failure_count == len(validation_result.source_results) else "completed"
        error_summary = _build_run_error_summary(total=len(validation_result.source_results), failures=source_failure_count)
        run_metadata = {
            "pipeline_stage": "validation_routing",
            "trigger_type": trigger_type,
            ...
            "routing_mode": validation_result.routing_mode,
            "validation_stats": validation_result.to_dict()["stats"],
        }
        candidate_updates_json_literal = _json_sql_literal(candidate_updates)
        review_tasks_json_literal = _json_sql_literal(review_tasks)
...
WITH candidate_payload AS (
    SELECT *
    FROM jsonb_to_recordset({candidate_updates_json_literal}::jsonb) AS payload(
        candidate_id text,
...
SET
    ...
    validation_issue_codes = payload.validation_issue_codes,
    updated_at = now()
FROM candidate_payload AS payload
WHERE nc.candidate_id = payload.candidate_id;

WITH review_task_payload AS (
    SELECT *
    FROM jsonb_to_recordset({review_tasks_json_literal}::jsonb) AS payload(
        review_task_id text,
        candidate_id text,
        run_id text,
        product_id text,
...
SET
    ...
    review_queued_count = :'review_queued_count'::integer,
...
"""
        self._execute(
            ...
            variables={
                ...
                "trigger_type": trigger_type,
                "triggered_by": triggered_by or "",
                "source_scope_count": str(len(validation_result.source_results)),
                "source_success_count": str(source_success_count),
                "source_failure_count": str(source_failure_count),
                "candidate_count": str(len(candidate_updates)),
                "review_queued_count": str(review_queued_count),
                "error_summary": error_summary or "",
                "partial_completion_flag": str(validation_result.partial_completion_flag).lower(),
                "completed_at": completed_at,
            },
        )
        return ValidationRoutingPersistenceResult(
            run_id=run_id,
            run_state=run_state,
            ...
            partial_completion_flag=validation_result.partial_completion_flag,
            completed_at=completed_at,
        )
...
"""
        ...
        if resolved:
            return resolved
        raise RuntimeError("Could not find validation/routing tables in the configured schema or in public.")
...
def _build_run_error_summary(*, total: int, failures: int) -> str | None:
    if failures == 0:
        return None
    if failures == total:
        return f"Validation/routing failed for all {total} sources."
    return f"Validation/routing failed for {failures} of {total} sources."
