...
class ValidationRoutingServiceTests(unittest.TestCase):
    ...
    def test_ai_identified_family_hub_cannot_auto_validate_as_one_product(self) -> None:
        temp_path = _prepare_workspace_temp_dir("validation-routing-ai-family-boundary")
        try:
            storage_config = ValidationRoutingStorageConfig(
                driver="filesystem",
                env_prefix="dev",
                ...
                **{
                    **input_item.__dict__,
                    "source_metadata": {
                        **input_item.source_metadata,
                        "discovery_metadata": {
                            "selection_reason_codes": ["hub_page_not_detail"]
                        },
                    },
                }
            )
    ...
    def test_verified_coverage_source_requires_review_for_any_product_family(self) -> None:
        temp_path = _prepare_workspace_temp_dir("validation-routing-verified-coverage")
        try:
            storage_config = ValidationRoutingStorageConfig(
                driver="filesystem",
                env_prefix="dev",
            ...
            result = service.validate_and_route_inputs(
                run_id="run-verified-coverage-001",
                inputs=[input_item],
                taxonomy_registry={
                    "savings": {
                        "standard",
            ...
            self.assertIn(
                "ambiguous_product_boundary",
                source_result.queue_reason_codes,
            )
        finally:
            rmtree(temp_path, ignore_errors=True)
