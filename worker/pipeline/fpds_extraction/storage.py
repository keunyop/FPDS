...
class ExtractionStorageConfig:
    driver: str
    env_prefix: str
    extraction_object_prefix: str
    retention_class: str
    ...
    @classmethod
    def from_env(cls) -> "ExtractionStorageConfig":
        return cls(
            ...
            extraction_object_prefix=os.getenv("FPDS_EXTRACTION_OBJECT_PREFIX", "extracted"),
    ...
    def build_extracted_object_key(
        self,
        *,
        country_code: str,
        bank_code: str,
...
def build_object_store(config: ExtractionStorageConfig) -> ParseChunkObjectStore:
    driver = config.driver.lower()
    if driver == "filesystem":
        if not config.filesystem_root:
            raise ValueError(
                "filesystem extraction storage requires FPDS_EXTRACTION_FILESYSTEM_ROOT or parse/snapshot filesystem root"
            )
        return FilesystemObjectStore(root_dir=Path(config.filesystem_root))
    if driver in {"s3-compatible", "s3"}:
        if not config.bucket:
            raise ValueError("s3-compatible extraction storage requires FPDS_OBJECT_STORAGE_BUCKET")
        return AwsCliS3ObjectStore(bucket=config.bucket, region=config.region, endpoint=config.endpoint)
    raise ValueError(f"Unsupported extraction storage driver: {config.driver}")
