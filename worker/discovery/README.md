...
- For domains in `FPDS_SOURCE_BROWSER_FALLBACK_DOMAINS`, snapshot capture may use a headless-browser PDF when direct HTTPS is blocked/timed out, an explicit rate page is only a JavaScript shell, or product HTML contains an unresolved dynamic rate placeholder. Recognized placeholder shapes include template functions, data-code elements, and double-bracket `[[...RATE...]]`/`[[...APR...]]` tokens. The fallback stays on the validated official URL, fails soft to usable direct HTML, and persists `fetch_method` plus the browser executable name in snapshot response metadata for auditability.
