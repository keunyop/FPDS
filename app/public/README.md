...
- the public header shows a simple FPDS logo mark, a medium-weight `FPDS` brand title, Home, Deposit, a disabled Loan entry, and a compact locale menu; Methodology remains available by route but is no longer a top-level header or Home action
- the public footer uses a Revolut-inspired structure with a compact brand row, route and coverage link groups, data-boundary copy, and the same compact locale menu pattern used by the header
