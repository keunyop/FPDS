...
- `/dashboard` renders a simplified public Home view with a market greeting, product count, bank count, interest-rate KPI, and card-backed ranked deposit comparisons with links into the Deposit catalog and product detail pages
...
- the public header shows a simple FPDS logo mark, a moderately weighted `FPDS` brand title, Home, Deposit, a disabled Loan entry, and a dropdown language selector; Methodology remains available by route but is no longer a top-level header menu item
