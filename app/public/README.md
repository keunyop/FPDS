...
- `/products` renders the simplified Deposit catalog against `GET /api/public/products` and `GET /api/public/filters`, with compact collapsible filter controls, sort controls between search conditions and the product list, small locally served real bank logos, official bank-page actions placed in the top-right of product cards when available, comparison-focused metric cards, and no product-card Changed/Verified date footer
- `/products/[productId]` renders public product facts from `GET /api/public/products/:productId`, includes a small locally served real bank logo, an estimated-interest calculator, source-derived signup/application facts, an optional period-rate table, a compact disclosure note, and a direct official bank product-page action when the public projection has a URL
- bank logo assets live under `public/bank-logos/`; RBC, CIBC, Scotiabank, and TD use official-site assets, BMO uses a small SVG logo asset with BMO identified as the author/source on Wikimedia Commons, and all are served locally to avoid repeated remote image fetches
