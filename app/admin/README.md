...
- protected `/admin/reviews/:reviewTaskId` review detail route with a simplified decision workspace: agent recommendation, origin source link, compact issue summary, editable reviewed values beside agent-proposed values, add-missing-field corrections, diff preview, and approve/reject/defer/edit-approve controls; evidence trace, model-run context, action history, and canonical context are still available in collapsed diagnostic sections
