...
- protected `/admin/reviews/:reviewTaskId` review detail route with a Public-detail-style candidate product summary: product identity, product-family-specific metrics, facts, and key conditions appear before the compact decision workspace; the same fields remain editable below with inline evidence, diff preview, and approve/reject/defer/edit-approve controls; evidence trace, model-run context, action history, and canonical context remain available in collapsed diagnostic sections
