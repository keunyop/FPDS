...
- protected `/admin/reviews` review queue with active-state defaults, search, registry-backed active-bank and Product Type filters, sort, pagination, direct snapshot stat cards, bounded table overflow, queued/deferred row multi-select with bulk defer only, and an operator-focused table that keeps task, bank/product, issue, confidence, validation, created time, state, and drill-in action visible while requiring evidence-aware approval or rejection from task detail
- protected `/admin/reviews/:reviewTaskId` review detail route with a simplified decision workspace: agent recommendation, origin source link, compact issue summary, generic candidate field/evidence coverage, missing expected-field focus, source-discovery role and rationale, editable reviewed values beside agent-proposed values, add-missing-field corrections, diff preview, and approve/reject/defer/edit-approve controls; evidence trace, model-run context, action history, and canonical context are still available in collapsed diagnostic sections
...
- admin table/list routes refresh visible server data automatically on a short interval, so run status and other operational list state changes appear without forcing operators to press `Search`; refresh pauses while the tab is hidden or a form control is being edited
- protected `/admin/runs/:runId` run detail route with operator-focused run counters, stage summary, source processing status, failure summary, related review-task links, usage summary, retry context, and a `Retry run` action for supported failed collection runs
...
- protected `/admin/usage` usage dashboard v1 with direct snapshot stat cards, bounded table overflow, search, provider/stage filters, by-model/by-agent/by-run concentration, trend deltas, and anomaly drilldowns focused on observed cost/token signals plus review/run actions
- protected `/admin/health/dashboard` dashboard health route with aggregate freshness, serving fallback, queue visibility, latest attempt status, and a manual retry action while hiding lower-level snapshot/request identifiers and explanatory definition blocks from the primary view
