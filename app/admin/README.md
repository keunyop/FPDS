...
- protected `/admin/reviews/:reviewTaskId` review detail route with a Public-detail-style candidate product summary followed by a problem-first correction workspace: only missing or suspect fields open by default, other collected fields stay in one collapsed group, source checking is reduced to confidence/evidence/open-source facts, and optional reason/note controls stay collapsed until needed; inline evidence, diff preview, approve/reject/defer/edit-approve controls, model context, history, and canonical context remain available
