...
- protected `/admin/reviews` review queue with active-state defaults, search, filters, sort, pagination, direct snapshot stat cards, bounded table overflow, and an operator-focused table that keeps task, bank/product, issue, confidence, validation, created time, state, and drill-in action visible while hiding lower-value country, updated, and candidate-state noise
